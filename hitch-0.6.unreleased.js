...
(function(hitch){
	...
	hitch.dom = {	
		...
		writeClasses: function(e,list){ e.className = list.join(" "); },
		hasMangledClass: function(list,i){
			return list.indexOf("_" + i) !== -1;
		},
		
		addMangledClass: function(t,i){
			var e = (t instanceof NodeList) ? t : [t], list, foundIndex;
			for(var x=0;x<e.length;x++){
				list = hitch.dom.parseClasses(e[x]);
				foundIndex = list.indexOf("_" + i);
				if(foundIndex === -1){
					list.push("_" +  i);
					(e[x].target || e[x]).className = list.join(" ");
				}
			}
		}, 
	...
	return function(src, finished, originNode){
		...
		src = src.replace(/\@-hitch-requires[^\;]*\;/g, function(m,i,s){
			...
			if(parts[1].indexOf('package:') === 0){
				requires.push({uri:'http://www.hitchjs.com/use/' + parts[1].substring(8) + ".js"});
			}else{
				requires.push({uri:parts[1]});
			}
			return '';
		});
